...
export function getMovies(page: number) { //Math.floor(Math.random()*10)+1
    return fetch(`${BASE_URL}/movie/now_playing?api_key=${API_KEY}&language=ko&page=${page}`)
            .then((res) => res.json())
}
...
export function getPopularMovie(){
    return fetch(`${BASE_URL}/movie/popular?api_key=${API_KEY}&language=ko&&page=1`)
            .then((res) => res.json())
}
