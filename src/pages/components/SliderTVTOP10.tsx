...
import { IdataTV, IresultsTV } from "../../api/apiTv";
...
function SliderTVTOP10 ({data, setClickTV, number}: ISliderTOP){
...
const likedArr:IresultsTV[] = useRecoilValue(likeTVState)
    ...
    return(
        ...
        <SliderMain ref={slideRef} >
            {data?.results.slice(0,10).map((movie,i) => 
                <Poster key={movie.id}>
                    <Flex onClick={()=>{openModal(movie.id); setClickTV(movie);}}>
